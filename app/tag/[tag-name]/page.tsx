...
export async function generateMetadata({ params }: { params: Promise<{ 'tag-name': string }> }): Promise<Metadata> {
  const resolvedParams = await params;
  const decodedTag = decodeURIComponent(resolvedParams['tag-name']);
  return {
    title: `Articles tagged with #${decodedTag} | RishtaMaker`,
    description: `Read the latest articles and guides tagged with #${decodedTag}.`,
  };
}
...
export default async function TagPage({ params }: { params: Promise<{ 'tag-name': string }> }) {
  const resolvedParams = await params;
  const decodedTag = decodeURIComponent(resolvedParams['tag-name']);
      ...
      <div className="container mx-auto px-4 py-16 sm:py-24 max-w-5xl">
        ...
        <div className="mb-12">
          ...
          <h1 className="text-4xl md:text-5xl font-[family-name:var(--font-display)] font-semibold text-slate-900 tracking-tight">
            Tag: <span className="text-slate-500">#{decodedTag}</span>
          </h1>
          <p className="mt-4 text-lg text-slate-600">
            Showing {posts.length} {posts.length === 1 ? 'post' : 'posts'} tagged with "{decodedTag}".
          </p>
        </div>
        ...
        {posts.length === 0 ? (
          <div className="text-center py-20 bg-slate-50 rounded-3xl border border-slate-200">
            ...
            <p className="text-slate-600 mt-2">Check back later for more content with this tag.</p>
          </div>
        ) : (
